...
export class EmployerService {
  public static getEmployers() {
    return pool.query<IEmployer>("SELECT * FROM employer")
  }
  public static postEmployer(employer: Omit<IEmployer, "id">) {
    const { name, degree, salary, job_id } = employer
    return pool.query<IEmployer, Array<IEmployer[keyof IEmployer]>>(`
      INSERT INTO employer (name, degree, salary, job_id) 
      VALUES ($1, $2, $3, $4) RETURNING *`,
      [name, degree, salary, job_id]
    )
  }
  public static getEmployer(id: string) {
    return pool.query<IEmployer, [string]>(`
  ...
  public static deleteEmployer(id: string, job_id?: string) {
    return pool.query<IEmployer, [string]>(`
